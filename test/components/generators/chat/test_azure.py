...
from haystack_experimental.components.generators.chat.azure import AzureOpenAIChatGenerator
from haystack_experimental.components.generators.chat.openai import _convert_message_to_openai_format
...
class TestAzureOpenAIChatGenerator:
    ...
    def test_init_fail_with_duplicate_tool_names(self, monkeypatch, tools):
        ...
        with pytest.raises(ValueError):
            AzureOpenAIChatGenerator(tools=duplicate_tools)
    ...
    #     assert data == {
    #         "type": "haystack_experimental.components.generators.chat.azure.AzureOpenAIChatGenerator",
    #         "init_parameters": {
    #             "api_key": {"env_vars": ["AZURE_OPENAI_API_KEY"], "strict": True, "type": "env_var"},
    #             "azure_deployment": "gpt-4o-mini",
    #             "azure_endpoint": 'test-endpoint',
    #             "organization": None,
    ...
    #         },
    #     }
    #     component = AzureOpenAIChatGenerator.from_dict(data)
    #     assert isinstance(component, AzureOpenAIChatGenerator)
    #     assert component.azure_deployment == "gpt-4o-mini"
    #     assert component.azure_endpoint == "test-endpoint"
    #     assert component.streaming_callback is print_streaming_chunk
    #     assert component.generation_kwargs == {"max_tokens": 10, "some_test_param": "test-params"}
    ...
    #     }
    #     with pytest.raises(ValueError):
    #         AzureOpenAIChatGenerator.from_dict(data)

    def test_run(self, chat_messages, mock_chat_completion):
        component = AzureOpenAIChatGenerator(api_key=Secret.from_token("test-api-key"), azure_deployment="gpt-4o-mini",
                                             azure_endpoint="test-endpoint", api_version="2023-05-15")
    ...
    @pytest.mark.skipif(
        ...
        reason=("Export an env var called AZURE_OPENAI_API_KEY and AZURE_OPENAI_ENDPOINT containing the Azure OpenAI "
                "API key and endpoint to run this test."),
    )
    ...
    def test_live_run_wrong_model(self, chat_messages):
        component = AzureOpenAIChatGenerator(azure_deployment="something-obviously-wrong")
    ...
    def test_live_run_streaming(self):
        ...
        callback = Callback()
        component = AzureOpenAIChatGenerator(streaming_callback=callback)
